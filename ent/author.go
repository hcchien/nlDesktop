...
func (e AuthorEdges) ImageOrErr() (*Photo, error) {
	if e.Image != nil {
		return e.Image, nil
...
func (*Author) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		...
		case author.FieldBio, author.FieldName:
			values[i] = new(sql.NullString)
		case author.FieldCreatedAt, author.FieldUpdatedAt:
			values[i] = new(sql.NullTime)
		case author.ForeignKeys[0]: // author_image
			values[i] = new(sql.NullInt64)
		default:
...
func (_m *Author) assignValues(columns []string, values []any) error {
	...
	for i := range columns {
		...
		case author.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field author_image", value)
			} else if value.Valid {
				_m.author_image = new(int)
				*_m.author_image = int(value.Int64)
			}
		default:
...
// QueryImage queries the "image" edge of the Author entity.
func (_m *Author) QueryImage() *PhotoQuery {
	return NewAuthorClient(_m.config).QueryImage(_m)
}

// QueryPosts queries the "posts" edge of the Author entity.
func (_m *Author) QueryPosts() *PostQuery {
	return NewAuthorClient(_m.config).QueryPosts(_m)
}
...
func (_m *Author) NamedPosts(name string) ([]*Post, error) {
...
func (_m *Author) appendNamedPosts(name string, edges ...*Post) {
	if _m.Edges.namedPosts == nil {
