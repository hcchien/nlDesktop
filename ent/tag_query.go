...
func (_q *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	for name, query := range _q.withNamedPosts {
		if err := _q.loadPosts(ctx, query, nodes,
			func(n *Tag) { n.appendNamedPosts(name) },
			func(n *Tag, e *Post) { n.appendNamedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *TagQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *TagQuery {
	query := (&PostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
