...
func (_q *CategoryQuery) QuerySection() *SectionQuery {
...
func (_q *CategoryQuery) WithSection(opts ...func(*SectionQuery)) *CategoryQuery {
	query := (&SectionClient{config: _q.config}).Query()
...
func (_q *CategoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Category, error) {
	...
	if query := _q.withSection; query != nil {
		if err := _q.loadSection(ctx, query, nodes, nil,
			func(n *Category, e *Section) { n.Edges.Section = e }); err != nil {
			return nil, err
		}
	}
...
func (_q *CategoryQuery) loadSection(ctx context.Context, query *SectionQuery, nodes []*Category, init func(*Category), assign func(*Category, *Section)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Category)
	for i := range nodes {
		if nodes[i].category_section == nil {
			continue
		}
		fk := *nodes[i].category_section
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "category_section" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *CategoryQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Category, init func(*Category), assign func(*Category, *Post)) error {
	...
	query.Where(func(s *sql.Selector) {
		...
		s.Join(joinT).On(s.C(post.FieldID), joinT.C(category.PostsPrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(category.PostsPrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(category.PostsPrimaryKey[1]))
...
func (_q *CategoryQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *CategoryQuery {
	query := (&PostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
