...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case author.Table:
		...
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, authorImplementors...); err != nil {
				return nil, err
			}
		}
	...
	case post.Table:
		...
		return query.Only(ctx)
	case section.Table:
		query := c.Section.Query().
			Where(section.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, sectionImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case author.Table:
		...
		query, err := query.CollectFields(ctx, authorImplementors...)
		if err != nil {
			return nil, err
		}
	...
	case section.Table:
		...
		query, err := query.CollectFields(ctx, sectionImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
