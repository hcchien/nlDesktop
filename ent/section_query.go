...
type SectionQuery struct {
	...
	loadTotal           []func(context.Context, []*Section) error
...
func (_q *SectionQuery) QueryCategories() *CategoryQuery {
...
func (_q *SectionQuery) QueryPosts() *PostQuery {
...
func (_q *SectionQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *SectionQuery) FirstIDX(ctx context.Context) int {
...
func (_q *SectionQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (_q *SectionQuery) OnlyIDX(ctx context.Context) int {
...
func (_q *SectionQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (_q *SectionQuery) IDsX(ctx context.Context) []int {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *SectionQuery) WithCategories(opts ...func(*CategoryQuery)) *SectionQuery {
	query := (&CategoryClient{config: _q.config}).Query()
...
func (_q *SectionQuery) WithPosts(opts ...func(*PostQuery)) *SectionQuery {
	query := (&PostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *SectionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Section, error) {
	...
	if query := _q.withCategories; query != nil {
		if err := _q.loadCategories(ctx, query, nodes,
			func(n *Section) { n.Edges.Categories = []*Category{} },
			func(n *Section, e *Category) { n.Edges.Categories = append(n.Edges.Categories, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withPosts; query != nil {
		if err := _q.loadPosts(ctx, query, nodes,
			func(n *Section) { n.Edges.Posts = []*Post{} },
			func(n *Section, e *Post) { n.Edges.Posts = append(n.Edges.Posts, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedCategories {
		if err := _q.loadCategories(ctx, query, nodes,
			func(n *Section) { n.appendNamedCategories(name) },
			func(n *Section, e *Category) { n.appendNamedCategories(name, e) }); err != nil {
			return nil, err
		}
	}
	for name, query := range _q.withNamedPosts {
		if err := _q.loadPosts(ctx, query, nodes,
			func(n *Section) { n.appendNamedPosts(name) },
			func(n *Section, e *Post) { n.appendNamedPosts(name, e) }); err != nil {
			return nil, err
		}
	}
...
func (_q *SectionQuery) loadCategories(ctx context.Context, query *CategoryQuery, nodes []*Section, init func(*Section), assign func(*Section, *Category)) error {
	...
	query.Where(predicate.Category(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(section.CategoriesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.category_section
		if fk == nil {
			return fmt.Errorf(`foreign-key "category_section" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "category_section" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *SectionQuery) loadPosts(ctx context.Context, query *PostQuery, nodes []*Section, init func(*Section), assign func(*Section, *Post)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Section)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Post(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(section.PostsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.post_section
		if fk == nil {
			return fmt.Errorf(`foreign-key "post_section" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "post_section" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *SectionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(section.Table, section.Columns, sqlgraph.NewFieldSpec(section.FieldID, field.TypeInt))
	_spec.From = _q.sql
...
func (_q *SectionQuery) WithNamedCategories(name string, opts ...func(*CategoryQuery)) *SectionQuery {
	query := (&CategoryClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *SectionQuery) WithNamedPosts(name string, opts ...func(*PostQuery)) *SectionQuery {
	query := (&PostClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
